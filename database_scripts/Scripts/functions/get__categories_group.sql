CREATE OR REPLACE FUNCTION prod.get__categories_group(_user_id bigint, _is_activ boolean DEFAULT NULL::boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 volatile 
AS $function$
...
then 
	select json_agg(json_build_object('id_category', id, 'name', name, 'is_activ', is_activ)) 
	   into result_json 
	FROM categories
	where user_id = _user_id AND is_group;
else 
	select json_agg(json_build_object('id_category', id, 'name', name, 'is_activ', is_activ)) 
	   into result_json 
	FROM categories 
	where user_id = _user_id AND is_group and is_activ = _is_activ;
end if;
