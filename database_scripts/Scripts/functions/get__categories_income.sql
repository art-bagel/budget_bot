CREATE OR REPLACE FUNCTION prod.get__categories_income(_user_id bigint, _is_activ boolean DEFAULT NULL::boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 volatile 
AS $function$
...
then 
	select json_agg(json_build_object('id_category', id, 'name', name, 'is_activ', is_activ)) 
	   into result_json 
	from categories 
	where user_id = _user_id AND is_income;
else 
	select json_agg(json_build_object('id_category', id, 'name', name, 'is_activ', is_activ)) 
	   into result_json 
	from categories 
	where user_id = _user_id AND is_income and is_activ = _is_activ;
end if;
