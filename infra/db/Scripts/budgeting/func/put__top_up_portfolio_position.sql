CREATE OR REPLACE FUNCTION budgeting.put__top_up_portfolio_position(
    ...
    _topped_up_at date DEFAULT CURRENT_DATE,
...
DECLARE
    ...
    _position_currency_code char(3);
    ...
    _lot record;
    _new_total_amount_in_base numeric(20, 2);
BEGIN
    ...
    IF _quantity IS NOT NULL AND _quantity <= 0 THEN
        RAISE EXCEPTION 'Top up quantity must be positive when provided';
    END IF;
    ...
    SELECT
        ...
        pp.quantity,
        pp.title,
        COALESCE((pp.metadata ->> 'amount_in_base')::numeric, 0)
    INTO
        _owner_type,
        _owner_user_id,
        _owner_family_id,
        ...
        _position_currency_code,
        _current_quantity,
        _title,
        _new_total_amount_in_base
    ...
    IF _status <> 'open' THEN
        RAISE EXCEPTION 'Only open positions can be topped up';
    END IF;

    IF _position_currency_code <> _currency_code THEN
        RAISE EXCEPTION 'Top up currency must match position currency';
    END IF;
    ...
    _operation_comment := 'Пополнение позиции · ' || _title;
    ...
    _new_total_amount_in_base := COALESCE(_new_total_amount_in_base, 0) + _cost_base;

    UPDATE portfolio_positions
    SET amount_in_currency = amount_in_currency + _amount_in_currency,
        quantity = CASE
            WHEN quantity IS NULL AND _quantity IS NULL THEN NULL
            WHEN quantity IS NULL THEN _quantity
            WHEN _quantity IS NULL THEN quantity
            ELSE quantity + _quantity
        END,
        metadata = jsonb_set(
            COALESCE(metadata, '{}'::jsonb),
            '{amount_in_base}',
            to_jsonb(_new_total_amount_in_base),
            true
        ),
    ...
    VALUES (
        _position_id,
        'top_up',
        COALESCE(_topped_up_at, CURRENT_DATE),
        _quantity,
        _amount_in_currency,
        _currency_code,
        _operation_id,
