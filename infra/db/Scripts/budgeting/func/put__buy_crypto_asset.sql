DROP FUNCTION IF EXISTS budgeting.put__buy_crypto_asset;
CREATE FUNCTION budgeting.put__buy_crypto_asset(
    _user_id bigint,
...
BEGIN
    ...
    PERFORM 1 FROM crypto_assets WHERE id = _crypto_asset_id;
    IF NOT FOUND THEN
    ...
    WHERE bank_account_id = _bank_account_id
      AND currency_code = _fiat_currency_code
    FOR UPDATE;
    ...
    IF _bank_balance < _fiat_amount THEN
        RAISE EXCEPTION 'Сумма превышает остаток';
    END IF;

    IF _fiat_currency_code = _base_currency_code THEN
        _cost_base := round(_fiat_amount, 2);
    ELSE
        _remaining_to_consume := _fiat_amount;
