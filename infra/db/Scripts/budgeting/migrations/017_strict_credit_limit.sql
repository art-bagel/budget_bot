-- Enforce credit limit unconditionally: remove the IS NOT NULL guard.
-- Previously, accounts with NULL credit_limit bypassed the check entirely.
-- Now:
--   - NULL credit_limit → error (limit not configured)
--   - exceeds limit     → error (limit exceeded)
-- Applies to both expense recording and transfers from credit accounts.
...
-- Same fix for transfers from credit accounts.
-- Replace the IS NOT NULL guard with a strict two-step check.
