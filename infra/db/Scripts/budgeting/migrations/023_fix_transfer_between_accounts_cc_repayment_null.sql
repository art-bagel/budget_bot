-- Fix cash -> investment/account transfers skipping budget entries.
--
-- Root cause: for non-credit target accounts _to_credit_kind is NULL, so
-- (_from_account_kind = 'cash' AND _to_credit_kind = 'credit_card') evaluates
-- to NULL, not false. PL/pgSQL IF treats NULL as not true, which skipped the
-- Unallocated debit for cash -> investment transfers.

DROP TRIGGER IF EXISTS trg_ensure_account_transfer_budget_entries
    ON budgeting.bank_entries;

DROP FUNCTION IF EXISTS budgeting.trg__ensure_account_transfer_budget_entries();
