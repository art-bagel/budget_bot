...
BEGIN
    ...
    ELSIF _currency_code = _base_currency_code THEN
        -- Family cash accounts may go negative (shared budget — overdraft mirrors how
        -- categories already may go negative). Personal cash accounts must stay >= 0.
        IF _bank_balance < _amount AND _bank_owner_type <> 'family' THEN
