...
- при переводе crypto в инвестиции можно указать текущую оценку в базовой валюте;
...
- доход по существующей crypto-позиции начисляется в сам актив: увеличивает количество токенов и фиксирует оценку дохода в базовой валюте;
- crypto-позицию в портфеле можно вручную переоценить;
- crypto-позиция подтягивает online price через CoinGecko и считает текущую стоимость как `online price × количество`;
- если online price недоступен, crypto-позиция использует последнюю ручную оценку;
- crypto icons берутся из одного frontend helper: `metadata.icon_url`, затем known CDN mappings;
- в списке портфеля crypto-позиция не показывает курс и себестоимость, эти детали доступны внутри позиции;
...
- системная категория `Unallocated` уменьшается на перенесенный cost basis, как при фиатном переводе денег в портфель;
...
- пользователь указывает количество полученной crypto и оценку дохода в базовой валюте;
- quantity позиции увеличивается на количество дохода;
- оценка дохода добавляется к cost basis/current value и `income_in_base`;
